...
//     setLoading(true);
//     try {
//       const res = await fetch(`/api/projects/${projectId}/generate/launch-plan`, { method: "POST" });
//       const json = await res.json();
//       if (!res.ok) { setMsg(json.message || "فشل التوليد"); }
//       else { onGenerated(json.launchPlan); setMsg(`✓ تم! رصيدك: ${json.creditsLeft}`); setTimeout(() => setMsg(""), 3000); }
//     } catch { setMsg("خطأ في الاتصال"); } finally { setLoading(false); }
//   };

//   if (!data) return (
//     <div className="card">
//       <div className="clabel">خطة الإطلاق - أول 90 يوم</div>
...
//       <div className="card">
//         <div className="clabel">نظرة عامة</div>
//         <p style={{ fontSize: ".88rem", color: "#C4BDB5", lineHeight: 1.8 }}>{data.overview}</p>
//       </div>

//       {[data.week1, data.month1, data.month2, data.month3].filter(Boolean).map((phase: any, pi: number) => (
//         <div key={pi} className="card">
//           <div className="clabel">{phase.title}</div>
//           {phase.goals?.length > 0 && (
//             <div style={{ display: "flex", flexWrap: "wrap", gap: ".375rem", marginBottom: ".875rem" }}>
//               {phase.goals.map((g: string, i: number) => (
//                 <span key={i} style={{ padding: ".25rem .65rem", borderRadius: 6, background: `${primary}15`, color: primary, border: `1px solid ${primary}33`, fontSize: ".72rem" }}>{g}</span>
//               ))}
//             </div>
//           )}
//           <div style={{ display: "flex", flexDirection: "column", gap: ".5rem" }}>
//             {(phase.tasks || []).map((t: any, i: number) => (
//               <div key={i} style={{ display: "grid", gridTemplateColumns: "90px 1fr auto auto", gap: ".5rem", alignItems: "center", padding: ".625rem", background: "#0A0A14", borderRadius: 9, border: "1px solid #1E1E2E" }}>
//                 <span style={{ fontSize: ".72rem", fontWeight: 700, color: primary }}>{t.day || t.week}</span>
//                 <span style={{ fontSize: ".8rem", color: "#C4BDB5" }}>{t.action}</span>
//                 <span style={{ fontSize: ".65rem", color: "#8A8498", background: "#1E1E2E", padding: ".15rem .4rem", borderRadius: 4, whiteSpace: "nowrap" }}>{t.platform}</span>
//                 <span style={{ fontSize: ".62rem", color: priorityColor(t.priority), background: priorityColor(t.priority) + "15", padding: ".15rem .4rem", borderRadius: 4, whiteSpace: "nowrap" }}>{t.priority}</span>
//               </div>
//             ))}
//           </div>
//         </div>
...
//       {data.kpis?.length > 0 && (
//         <div className="card">
//           <div className="clabel">مؤشرات الأداء KPIs</div>
//           <div style={{ display: "flex", flexDirection: "column", gap: ".5rem" }}>
//             {data.kpis.map((k: any, i: number) => (
//               <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: ".75rem", padding: ".75rem", background: "#0A0A14", borderRadius: 9, border: "1px solid #1E1E2E" }}>
//                 <span style={{ fontSize: ".82rem", color: "#C4BDB5" }}>{k.metric}</span>
//                 <span style={{ fontSize: ".78rem", fontWeight: 700, color: primary }}>{k.target}</span>
//                 <span style={{ fontSize: ".68rem", color: "#8A8498" }}>{k.timeline}</span>
//               </div>
//             ))}
//           </div>
//         </div>
...
//       {data.budget && (
//         <div className="card">
//           <div className="clabel">توزيع الميزانية المقترحة — {data.budget.total}</div>
//           {data.budget.breakdown?.map((b: any, i: number) => (
//             <div key={i} style={{ display: "flex", alignItems: "center", gap: ".75rem", marginBottom: ".625rem" }}>
//               <span style={{ fontSize: ".78rem", color: "#8A8498", width: 160, textAlign: "right", flexShrink: 0 }}>{b.category}</span>
//               <div style={{ flex: 1, height: 8, background: "#1E1E2E", borderRadius: 4, overflow: "hidden" }}>
//                 <div style={{ height: "100%", background: `linear-gradient(90deg,${primary},${primary}88)`, width: `${b.pct}%`, borderRadius: 4 }} />
//               </div>
//               <span style={{ fontSize: ".75rem", fontWeight: 700, color: primary, width: 30 }}>{b.pct}%</span>
//             </div>
//           ))}
//         </div>
//       )}

//       <button onClick={generate} disabled={loading} style={{ width: "100%", padding: ".625rem", borderRadius: 10, background: "transparent", border: `1px solid ${primary}33`, color: primary, fontSize: ".8rem", cursor: "pointer", marginTop: ".5rem" }}>
