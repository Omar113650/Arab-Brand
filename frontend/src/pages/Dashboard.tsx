...
/* ── RESULT SCREEN ── */
// ── الـ TABS الكاملة مع الإضافات الجديدة ──
const TABS = [
  { id: "identity", label: "🎨 الهوية" },
  { id: "logo", label: "🏷️ الشعار" },
  { id: "social", label: "📱 السوشيال" },
  ...
  { id: "email", label: "📧 حملة إيميل" },
  { id: "competitors", label: "🔍 المنافسون" },
  { id: "businessOverview", label: "🏢 شرح البيزنس" }, // ← جديد
  { id: "agePreferences", label: "🎯 تفضيلات الأجيال" }, // ← جديد
  { id: "faq", label: "❓ الأسئلة الشائعة" }, // ← جديد
];
...
function ResultScreen({
  ...
  const [persona, setPersona] = useState<any>(result?.buyerPersona || null);
  const [adScripts, setAdScripts] = useState<any>(result?.adScripts || null);
  const [emailCamp, setEmailCamp] = useState<any>(
    result?.emailCampaign || null,
  );
  // ── الثلاثة الجديدة ──
  const [businessOverview, setBusinessOverview] = useState<any>(
    result?.businessOverview || null,
  );
  const [agePreferences, setAgePreferences] = useState<any>(
    result?.agePreferences || null,
  );
      ...
      <div className="wrap" style={{ maxWidth: 800 }}>
        ...
        {tab === "businessOverview" && (
          <BusinessOverviewTab
            ...
            projectId={projectId}
            primary={primary}
            onGenerated={setBusinessOverview}
          />
        )}
        {tab === "agePreferences" && (
          <AgePreferencesTab
...
function SocialTab({
  ...
  const handleGenerateMore = async () => {
    ...
    setGenMsg("جاري توليد محتوى إضافي...");
    try {
      const res = await fetch(`/api/projects/${projectId}/extra-social`, {
        method: "POST",
      });
      const data = await res.json();
      if (!res.ok) {
...
/* ── HELPER: Paid Generate Button ── */
function PaidGenerateBtn({
  onGenerate,
  loading,
  label = "توليد بـ 1 كريديت",
}: {
  onGenerate: () => void;
  loading: boolean;
  label?: string;
    ...
    <div style={{ textAlign: "center", padding: "3rem 2rem" }}>
      ...
      <button
        ...
        style={{
          ...
          borderRadius: 14,
          background: loading ? "#1E1E2E" : "#C9973A",
          color: loading ? "#8A8498" : "#08080F",
          border: "none",
          fontFamily: "Tajawal,sans-serif",
          fontSize: "1rem",
          fontWeight: 700,
      ...
      <p style={{ fontSize: ".7rem", color: "#3A3650", marginTop: ".75rem" }}>
        يُخصم 1 كريديت من رصيدك
      </p>
    </div>
  );
}
  ...
  return (
    <div className="fade-up">
      ...
      <div className="card" style={{ marginTop: "1rem" }}>
        ...
        >
          انسخ الكود أدناه وأضفه في إعدادات توقيع بريدك الإلكتروني
        </p>
        <pre
          style={{
            background: "#0A0A14",
        ...
        <button
          ...
          style={{ marginTop: ".75rem", fontSize: ".75rem" }}
          onClick={() => {
  ...
  const generate = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${projectId}/regenerate/launchPlan`,
        { method: "POST" },
      );
      const json = await res.json();
      if (!res.ok) {
        setMsg(json.message || "فشل التوليد");
      } else {
        onGenerated(json.launchPlan);
        setMsg("✓ تم إعادة التوليد");
    ...
    <div className="fade-up">
      ...
      {data.overview && (
        <div className="card">
          ...
          >
            {[
              ["المدة الإجمالية", data.overview.totalDuration],
              ["الميزانية المقترحة", data.overview.budget],
            ]
              .filter(([, v]) => v)
              .map(([label, val]) => (
                <div
      ...
      {data.phases?.map((phase: any, pi: number) => (
        <div key={pi} className="card">
          ...
          >
            ...
            <div>
              <div
                style={{ fontSize: ".9rem", fontWeight: 700, color: "#F0EDE6" }}
              >
                {phase.name}
              </div>
              <div style={{ fontSize: ".72rem", color: "#8A8498" }}>
            ...
            <div style={{ marginBottom: ".75rem" }}>
              ...
              >
                {phase.tasks.map((t: any, i: number) => (
                  ...
                  >
                    <span style={{ fontSize: ".8rem", color: "#C4BDB5" }}>
                      {t.task || t}
                    </span>
                    {t.owner && (
                      <span
          ...
          >
            {data.channels.map((ch: any, i: number) => (
              ...
              >
                ...
                <span style={{ fontSize: ".78rem", color: "#C4BDB5" }}>
                  {ch.strategy}
                </span>
                {ch.budget && (
                  <span
              ...
              {data.contingency.criticalRisks.map((r: string, i: number) => (
                <div
                  ...
                  style={{
                    ...
                    padding: ".3rem 0",
                    borderBottom:
                      i < data.contingency.criticalRisks.length - 1
                        ? "1px solid #1E1E2E"
                        : "none",
                  }}
                >
...
function SwotTab({
  ...
  const generate = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/projects/${projectId}/regenerate/swot`, {
        method: "POST",
      });
      const json = await res.json();
      if (!res.ok) {
                ...
                {items.length === 0 ? (
                  ...
                  items.map((item: any, i: number) => (
                    <div
                      ...
                      style={{
                        borderBottom:
                          i < items.length - 1 ? "1px solid #1E1E2E" : "none",
                        paddingBottom: i < items.length - 1 ? ".5rem" : 0,
                      }}
                    >
                      <p
                        style={{
                      ...
                      {item.impact && (
                        <span style={{ fontSize: ".6rem", color: "#8A8498" }}>
                          التأثير: {item.impact}
                        </span>
                      )}
                    </div>
                ...
                >
                  ...
                  >
                    ...
                    {r.impact && (
                      <div style={{ fontSize: ".72rem", color: "#8A8498" }}>
                        التأثير:{" "}
                        <span style={{ color: riskColor }}>{r.impact}</span>
                      </div>
                    )}
                  </div>
                  {r.mitigation && (
          ...
          {data.verdict && (
            <div
              style={{
                ...
                padding: "1rem",
                background:
                  data.verdict.goOrNoGo === "go"
                    ? "#4ADE8011"
                    : data.verdict.goOrNoGo === "no-go"
                      ? "#F8717111"
                      : "#FBBF2411",
                borderRadius: 12,
                border: `1px solid ${data.verdict.goOrNoGo === "go" ? "#4ADE8033" : data.verdict.goOrNoGo === "no-go" ? "#F8717133" : "#FBBF2433"}`,
              }}
            >
              <div style={{ textAlign: "center", flexShrink: 0 }}>
          ...
          {data.criticalSuccessFactors?.length > 0 && (
            <div style={{ marginTop: "1rem" }}>
              ...
              {data.criticalSuccessFactors.map((f: string, i: number) => (
                <div
                  key={i}
                  style={{ display: "flex", gap: ".5rem", padding: ".4rem 0" }}
                >
                  <span style={{ color: primary }}>★</span>
                  <span style={{ fontSize: ".82rem", color: "#C4BDB5" }}>
                    {f}
...
function ObjectionsTab({
  ...
  const generate = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${projectId}/regenerate/objections`,
        { method: "POST" },
      );
      const json = await res.json();
      if (!res.ok) {
    ...
    <div className="fade-up">
      ...
      {data.salesVoice && (
        <div className="card" style={{ borderColor: `${primary}33` }}>
          ...
          <p style={{ fontSize: ".85rem", color: "#C4BDB5", lineHeight: 1.7 }}>
            {data.salesVoice}
          </p>
        </div>
      )}
        ...
        return (
          ...
          >
            ...
            >
              ...
              <span style={{ color: "#8A8498", fontSize: ".9rem" }}>
                {isOpen ? "▲" : "▼"}
              </span>
            </div>
              ...
              >
                ...
                {obj.keyPoints?.filter(Boolean).length > 0 && (
                  <div style={{ marginBottom: ".625rem" }}>
                    ...
                    >
                      نقاط الإقناع الرئيسية:
                    </div>
                    {obj.keyPoints
                      .filter(Boolean)
                      .map((kp: string, j: number) => (
                        <div
                          key={j}
                          style={{
                            display: "flex",
...
function BuyerPersonaTab({
  ...
  const generate = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${projectId}/generate/buyer-persona`,
        { method: "POST" },
      );
      const json = await res.json();
      if (!res.ok) {
        setMsg(json.message || "فشل");
      } else {
        onGenerated(json.buyerPersona);
        setMsg(`✓ تم! رصيدك: ${json.creditsLeft}`);
        setTimeout(() => setMsg(""), 3000);
      }
    } catch {
      setMsg("خطأ في الاتصال");
  ...
  const PersonaCard = ({ p, isPrimary }: { p: any; isPrimary: boolean }) => (
    <div
      style={{
        background: isPrimary ? `${primary}08` : "#0A0A14",
        border: `1px solid ${isPrimary ? primary + "33" : "#1E1E2E"}`,
        borderRadius: 16,
        padding: "1.25rem",
        marginBottom: "1rem",
      }}
    >
      ...
      >
        <div
          style={{
            ...
            height: 56,
            borderRadius: "50%",
            background: isPrimary ? primary : "#1E1E2E",
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            fontSize: "1.75rem",
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: ".5rem" }}>
            <span
              style={{ fontSize: "1rem", fontWeight: 700, color: "#F0EDE6" }}
            >
              {p.name}
            </span>
            {isPrimary && (
          ...
          <div
            style={{ fontSize: ".78rem", color: "#8A8498", marginTop: ".2rem" }}
          >
            {p.age} · {p.gender} · {p.location}
          </div>
          <div style={{ fontSize: ".75rem", color: primary }}>
            {p.job} · {p.income}
          </div>
        </div>
      </div>
      ...
      {p.buyingBehavior && (
        ...
        >
          ...
          <p style={{ fontSize: ".78rem", color: "#C4BDB5", margin: 0 }}>
            {p.buyingBehavior}
          </p>
        </div>
      )}
  ...
  return (
    <div className="fade-up">
      ...
      {data.primary && <PersonaCard p={data.primary} isPrimary={true} />}
      {data.secondary?.name && (
        <PersonaCard p={data.secondary} isPrimary={false} />
      )}
      {data.insights?.length > 0 && (
        <div className="card">
          <div className="clabel">رؤى تسويقية مهمة</div>
          {data.insights.map((ins: string, i: number) => (
            <div
              ...
              style={{
                ...
                padding: ".5rem 0",
                borderBottom:
                  i < data.insights.length - 1 ? "1px solid #1E1E2E" : "none",
              }}
            >
              <span style={{ color: primary }}>💡</span>
              <span
...
function AdScriptsTab({
  ...
  const generate = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${projectId}/generate/ad-scripts`,
        { method: "POST" },
      );
      const json = await res.json();
      if (!res.ok) {
    ...
    return (
      <div className="card">
        ...
        <PaidGenerateBtn
          onGenerate={generate}
          loading={loading}
          label="توليد سكريبتات الإعلانات"
        />
      </div>
    );
      ...
      {data.scripts?.map((s: any, i: number) => (
        <div key={i} className="card">
          <div
            ...
            onClick={() => setOpenScript(openScript === i ? -1 : i)}
          >
            <div
              style={{ display: "flex", alignItems: "center", gap: ".625rem" }}
            >
              ...
              <div>
                ...
                <div style={{ fontSize: ".68rem", color: "#8A8498" }}>
                  {s.duration} · {s.type}
                </div>
              </div>
            </div>
          ...
          {openScript === i && (
            <div style={{ marginTop: "1rem" }}>
              {[
                ["⚡ الجملة الافتتاحية", s.hook, "#C9973A"],
                ["😤 المشكلة", s.problem, "#F87171"],
                ["✨ الحل", s.solution, "#4ADE80"],
                ["📢 نداء الإجراء", s.cta, primary],
              ].map(([label, val, color]) =>
                val ? (
                  <div
                    key={label}
...
function EmailCampaignTab({
  ...
  const generate = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${projectId}/generate/email-campaign`,
        { method: "POST" },
      );
      const json = await res.json();
      if (!res.ok) {
        ...
        onGenerated(json.emailCampaign);
        setMsg(`✓ تم! رصيدك: ${json.creditsLeft}`);
        setTimeout(() => setMsg(""), 3000);
      }
    } catch {
        ...
        {data.sequence?.map((_: any, i: number) => (
          <button
            ...
            style={{
              padding: ".4rem .9rem",
              borderRadius: 20,
              border: `1.5px solid ${openEmail === i ? primary : "#1E1E2E"}`,
              background: openEmail === i ? primary : "transparent",
              color: openEmail === i ? "#08080F" : "#8A8498",
              fontFamily: "Tajawal,sans-serif",
              fontSize: ".78rem",
              fontWeight: openEmail === i ? 700 : 400,
              cursor: "pointer",
            }}
          >
            إيميل {i + 1}
          ...
          openEmail === i && (
            <div key={i} className="card">
              ...
              >
                ...
                <p style={{ fontSize: ".8rem", color: "#8A8498" }}>
                  {email.body?.closing}
                </p>
              </div>
            </div>
          ...
          {data.tips.map((tip: string, i: number) => (
            <div
              ...
              style={{
                ...
                padding: ".5rem 0",
                borderBottom:
                  i < data.tips.length - 1 ? "1px solid #1E1E2E" : "none",
              }}
            >
              <span style={{ color: primary }}>💡</span>
              <span
...
/* ══════════════════════════════════════════════════════
   ── الـ TABS الجديدة الثلاثة ──
══════════════════════════════════════════════════════ */

/* ── BUSINESS OVERVIEW TAB ← جديد ── */
function BusinessOverviewTab({
  ...
  const generate = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${projectId}/regenerate/businessOverview`,
        { method: "POST" },
      );
      const json = await res.json();
      if (!res.ok) {
        ...
        onGenerated(json.businessOverview);
        setMsg("✓ تم إعادة التوليد");
        setTimeout(() => setMsg(""), 3000);
      }
    } catch {
  ...
  if (!data || Object.keys(data).length === 0)
    return (
      <div className="card">
        <div className="clabel">شرح البيزنس الاحترافي</div>
        {msg && (
          <div
            style={{
              background: "#F8717111",
  ...
  return (
    <div className="fade-up">
      ...
      {data.elevator_pitch && (
        <div
          ...
          style={{
            ...
            background: `linear-gradient(135deg, ${primary}08, #0E0E1A)`,
          }}
        >
          <div
            style={{
      ...
      {data.what_we_do && (
        <div className="card">
          <div className="clabel">ماذا نفعل بالضبط؟</div>
          <div
            style={{
              background: "#0A0A14",
              borderRadius: 10,
          ...
          >
            ...
            >
              {data.what_we_do.summary}
            </p>
          </div>
          {data.what_we_do.details?.length > 0 && (
            <div
              style={{ display: "flex", flexDirection: "column", gap: ".4rem" }}
            >
              {data.what_we_do.details.map((d: string, i: number) => (
                <div
                  key={i}
                  style={{
                    display: "flex",
                ...
                >
                  <span style={{ color: primary, flexShrink: 0, marginTop: 2 }}>
                    →
                  </span>
                  <span
                    style={{
      ...
      {data.who_we_serve && (
        <div className="card">
          <div className="clabel">من نخدم؟</div>
          <div
            style={{ display: "flex", flexDirection: "column", gap: ".625rem" }}
          >
            {[
              ["🎯 الشريحة الأساسية", data.who_we_serve.primary, "#4ADE80"],
              ["👥 الشريحة الثانوية", data.who_we_serve.secondary, "#60A5FA"],
              ["🚫 ليس مناسباً لـ", data.who_we_serve.notFor, "#F87171"],
            ]
              .filter(([, v]) => v)
              .map(([label, val, color]) => (
                <div
                  ...
                  style={{
                    ...
                    borderRight: `3px solid ${color as string}`,
                  }}
                >
                  <div
                    style={{
      ...
      {/* Problems We Solve */}
      {data.problems_we_solve?.length > 0 && (
        <div className="card">
          <div className="clabel">المشاكل التي نحلها</div>
          <div
            style={{ display: "flex", flexDirection: "column", gap: ".75rem" }}
          >
            {data.problems_we_solve.map((p: any, i: number) => (
              <div
                key={i}
                style={{
                  background: "#0A0A14",
      ...
      {data.why_choose_us?.length > 0 && (
        <div className="card">
          <div className="clabel">لماذا يختارونا؟</div>
          <div
            style={{ display: "flex", flexDirection: "column", gap: ".75rem" }}
          >
            {data.why_choose_us.map((w: any, i: number) => (
              <div
                key={i}
                style={{
                  background: "#0A0A14",
              ...
              >
                ...
                {w.vsCompetitor && (
                  <p
                    style={{ fontSize: ".75rem", color: "#60A5FA", margin: 0 }}
                  >
                    مقارنة بالمنافسين: {w.vsCompetitor}
                  </p>
                )}
              </div>
            ))}
      ...
      {data.unique_value && (
        <div className="card">
          <div className="clabel">القيمة الفريدة والرؤية</div>
          {data.unique_value.statement && (
            <div
              style={{
      ...
      {data.business_model && (
        <div className="card">
          <div className="clabel">نموذج العمل التجاري</div>
          <div
            style={{ display: "flex", flexDirection: "column", gap: ".625rem" }}
          >
            {[
              ["💰 كيف نكسب المال", data.business_model.howWeEarnMoney],
              ["🏷️ فلسفة التسعير", data.business_model.pricingApproach],
              ["📈 قابلية التوسع", data.business_model.scalability],
            ]
              .filter(([, v]) => v)
              .map(([label, val]) => (
                <div
      ...
      {/* Social Proof */}
      {data.social_proof_potential?.length > 0 && (
        <div className="card">
          <div className="clabel">أنواع الإثبات الاجتماعي المناسبة</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: ".375rem" }}>
            {data.social_proof_potential.map((sp: string, i: number) => (
              <span
                key={i}
                style={{
                  padding: ".3rem .75rem",
...
/* ── AGE PREFERENCES TAB ← جديد ── */
function AgePreferencesTab({
  data,
  projectId,
  primary,
  onGenerated,
  ...
  const generate = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${projectId}/regenerate/agePreferences`,
        { method: "POST" },
      );
      const json = await res.json();
      if (!res.ok) {
        ...
        onGenerated(json.agePreferences);
        setMsg("✓ تم إعادة التوليد");
        setTimeout(() => setMsg(""), 3000);
      }
    } catch {
  ...
  if (!data || Object.keys(data).length === 0)
    return (
      <div className="card">
        <div className="clabel">تفضيلات الأجيال وتوصيات التسويق</div>
        {msg && (
          <div
            style={{
              background: "#F8717111",
  ...
  return (
    <div className="fade-up">
      ...
      {data.priorityAction && (
        <div className="card">
          <div className="clabel">أولويات التسويق</div>
          <div
            style={{
              display: "grid",
          ...
          >
            {[
              ["⚡ أسرع ربح", data.priorityAction.quickWin, "#4ADE80"],
              ["💰 أعلى عائد", data.priorityAction.highestROI, "#C9973A"],
              ["🔭 رهان المستقبل", data.priorityAction.longTermBet, "#A78BFA"],
            ].map(([label, val, color]) => (
              <div
                key={label}
                style={{
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  ...
                  >
                    {group.generationName}
                  </span>
                </div>
                <div style={{ display: "flex", gap: ".375rem" }}>
                  <span style={{ fontSize: ".68rem", color: "#8A8498" }}>
                    حجم السوق:
                  </span>
                  <span
                    style={{ fontSize: ".68rem", color: gc, fontWeight: 600 }}
                  >
                    {group.marketSize}
                  </span>
                  <span style={{ fontSize: ".68rem", color: "#3A3650" }}>
                    |
                  </span>
                  <span style={{ fontSize: ".68rem", color: "#8A8498" }}>
                    القوة الشرائية:
                  ...
                  <span
                    style={{
                      fontSize: ".68rem",
                      color: powerColors[group.buyingPower] || "#8A8498",
                      fontWeight: 600,
                    }}
                  >
                    {group.buyingPower}
              ...
              >
                ...
                {group.preferences && (
                  <div style={{ marginBottom: ".75rem" }}>
                    ...
                    >
                      {group.preferences.whatTheyLove?.length > 0 && (
                        <div
                          style={{
                            background: "#08080F",
                            borderRadius: 9,
                        ...
                        >
                          ...
                          {group.preferences.whatTheyLove.map(
                            (l: string, j: number) => (
                              <div
                                key={j}
                                style={{
                      ...
                      {group.preferences.whatTheyHate?.length > 0 && (
                        <div
                          style={{
                            background: "#08080F",
                            borderRadius: 9,
                        ...
                        >
                          ...
                          {group.preferences.whatTheyHate.map(
                            (h: string, j: number) => (
                              <div
                                key={j}
                                style={{
                    ...
                    {group.preferences.pricesSensitivity && (
                      <div
                        style={{
                          background: "#08080F",
                          borderRadius: 9,
                ...
                {group.contentThatWorks && (
                  ...
                  >
                    ...
                    >
                      ...
                      {group.contentThatWorks.platforms?.length > 0 && (
                        <div>
                          <div
                            style={{
                              fontSize: ".6rem",
                          ...
                          >
                            {group.contentThatWorks.platforms.map(
                              (p: string, j: number) => (
                                <span
                                  key={j}
                                  style={{
                    ...
                    {group.contentThatWorks.hooks?.length > 0 && (
                      <div>
                        <div
                          style={{
                            fontSize: ".6rem",
                        ...
                        >
                          جمل الافتتاح التي تجذبهم
                        </div>
                        {group.contentThatWorks.hooks.map(
                          (h: string, j: number) => (
                            <div
                              key={j}
                              style={{
                                ...
                                padding: ".25rem 0",
                                borderBottom:
                                  j < group.contentThatWorks.hooks.length - 1
                                    ? "1px solid #1E1E2E"
                                    : "none",
                              }}
                            >
                ...
                {group.recommendations?.length > 0 && (
                  <div style={{ marginBottom: ".75rem" }}>
                    ...
                    {group.recommendations.map((rec: any, j: number) => (
                      <div
                        key={j}
                        style={{
                          background: `${gc}08`,
                      ...
                      >
                        ...
                        >
                          {rec.timeline && (
                            <div
                              style={{ fontSize: ".68rem", color: "#FBBF24" }}
                            >
                              ⏱ {rec.timeline}
                            </div>
                          )}
                          {rec.expectedResult && (
                            <div
                              style={{ fontSize: ".68rem", color: "#4ADE80" }}
                            >
                              📈 {rec.expectedResult}
                            </div>
                          )}
                ...
                {group.productIdeas?.length > 0 && (
                  <div style={{ marginBottom: ".5rem" }}>
                    ...
                    >
                      {group.productIdeas.map((p: string, j: number) => (
                        <span
                          key={j}
                          style={{
                            padding: ".25rem .65rem",
      ...
      {/* Cross Generational */}
      {data.crossGenerational && (
        <div className="card">
          <div className="clabel">القاسم المشترك بين الأجيال</div>
          <div
            style={{ display: "flex", flexDirection: "column", gap: ".625rem" }}
          >
            {data.crossGenerational.commonGround && (
              <div
                style={{
                  background: "#0A0A14",
                  borderRadius: 9,
            ...
            {data.crossGenerational.mainTension && (
              <div
                style={{
                  background: "#0A0A14",
                  borderRadius: 9,
            ...
            {data.crossGenerational.unifyingMessage && (
              <div
                style={{
                  background: `${primary}08`,
                  borderRadius: 9,
...
function FaqTab({
  ...
  const generate = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/projects/${projectId}/regenerate/faq`, {
        method: "POST",
      });
      const json = await res.json();
      if (!res.ok) {
  ...
  if (!data || Object.keys(data).length === 0)
    return (
      <div className="card">
        <div className="clabel">الأسئلة الشائعة</div>
        {msg && (
          <div
            style={{
              background: "#F8717111",
  ...
  const filtered =
    filterCat === "الكل"
      ? faqs
      : faqs.filter((f: any) => f.category === filterCat);
  ...
  return (
    <div className="fade-up">
      ...
      {data.insights && (
        <div className="card" style={{ borderColor: `${primary}33` }}>
          <div className="clabel">رؤى من الأسئلة الشائعة</div>
          {data.insights.topConcern && (
            <div
              style={{
                background: "#F8717108",
                borderRadius: 9,
            ...
            >
              ...
              >
                🔴 أكبر قلق لدى العملاء
              </div>
              <p style={{ fontSize: ".85rem", color: "#C4BDB5", margin: 0 }}>
                {data.insights.topConcern}
              </p>
            </div>
          )}
          ...
          >
            {data.insights.contentGaps?.length > 0 && (
              <div
                style={{
                  background: "#0A0A14",
                  borderRadius: 9,
              ...
              >
                ...
                {data.insights.contentGaps.map((g: string, i: number) => (
                  <div
                    key={i}
                    style={{
                      fontSize: ".72rem",
            ...
            {data.insights.salesOpportunities?.length > 0 && (
              <div
                style={{
                  background: "#0A0A14",
                  borderRadius: 9,
              ...
              >
                ...
                {data.insights.salesOpportunities.map(
                  (o: string, i: number) => (
                    <div
                      key={i}
                      style={{
          ...
          {categories.map((cat: string) => (
            <button
              ...
              style={{
                padding: ".3rem .7rem",
                borderRadius: 16,
                border: `1px solid ${filterCat === cat ? primary : "#1E1E2E"}`,
                background: filterCat === cat ? `${primary}22` : "transparent",
                color: filterCat === cat ? primary : "#8A8498",
                fontSize: ".72rem",
                cursor: "pointer",
              }}
            >
      ...
      {filtered.map((faq: any, i: number) => {
        const isOpen = openIdx === i;
        const tc = toneColor[faq.tone] || primary;
        return (
          <div
            key={i}
            className="card"
